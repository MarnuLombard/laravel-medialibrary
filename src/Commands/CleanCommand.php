...
class CleanCommand extends Command
{
     ...
     */
    public function __construct(MediaRepository $mediaRepository, FileManipulator $fileManipulator, Factory $fileSystem, BasePathGenerator $basePathGenerator)
    {
        parent::__construct();
    ...
    protected function deleteFilesGeneratedForDeprecatedConversions()
    {
        $this->getMediaItems()->each(function (Media $media) {
            ...
            collect($currentFilePaths)->filter(function ($currentFilePath) use($conversionFilePaths) {
                return !$conversionFilePaths->contains(basename($currentFilePath));
            })->each(function ($currentFilePath) use($media) {
                if (!$this->isDryRun) {
    ...
    protected function deleteOrphanedFiles()
    {
        ...
        collect($this->fileSystem->disk($diskName)->directories())->filter(function ($directory) use($mediaIds) {
            return is_numeric($directory) ? !$mediaIds->contains((int) $directory) : false;
        })->each(function ($directory) use($diskName) {
            if (!$this->isDryRun) {
